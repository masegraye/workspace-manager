...
func NewCommitCommandV2() *cobra.Command {
	var (
		message      string
		interactive  bool
		addAll       bool
...
Examples:
  ...
  wsm commit-v2 -m "feat: add new feature"

  # Commit changes in specific workspace
  wsm commit-v2 /path/to/workspace -m "fix: resolve issue"

  # Use commit message template
  wsm commit-v2 --template feature -m "implement user authentication"

  # Add all files and commit
  wsm commit-v2 -m "chore: update dependencies" --add-all

  # Commit and push changes
  wsm commit-v2 -m "docs: update README" --push

  # Dry run to see what would be committed
  wsm commit-v2 -m "test commit" --dry-run

  # Interactive mode for file selection
  wsm commit-v2 --interactive

  # Commit only specific repositories
  wsm commit-v2 -m "fix: auth service" --repositories auth,shared

  # JSON output for scripting
  wsm commit-v2 -m "feat: new endpoint" --json`,
