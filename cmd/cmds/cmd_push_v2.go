...
func NewPushCommandV2() *cobra.Command {
	var (
		workspace    string
		dryRun       bool
		force        bool
	...
	cmd := &cobra.Command{
		Use:   "push-v2 <remote-name> [workspace-path]",
...
Examples:
  ...
  wsm push-v2 fork my-workspace --dry-run

  # Push to fork remote interactively
  wsm push-v2 fork my-workspace

  # Push all branches without asking
  wsm push-v2 fork my-workspace --force

  # Push and set upstream tracking
  wsm push-v2 fork my-workspace --set-upstream

  # Push only specific repositories
  wsm push-v2 fork --repositories auth,shared

  # JSON output for scripting
  wsm push-v2 fork --json`,
