...
func NewMergeCommandV2() *cobra.Command {
	var (
		dryRun        bool
		force         bool
		workspace     string
...
func runMergeV2(ctx context.Context, workspaceName string, req service.MergeRequest, jsonOut bool) error {
	...
	if workspaceName == "" {
		...
		if err != nil {
			return errors.Wrap(err, "failed to detect workspace. Use 'wsm merge-v2 <workspace-name>' or specify --workspace flag")
		}
