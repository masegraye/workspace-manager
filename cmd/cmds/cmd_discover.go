...
Examples:
  ...
  wsm discover ~/code ~/projects --max-depth 2

  # Non-recursive discovery
  wsm discover ~/code --recursive=false

  # Verbose output showing categories detected
  wsm discover ~/code --verbose`,
		Args: cobra.MinimumNArgs(0),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runDiscover(cmd.Context(), args, recursive, maxDepth, verbose)
		},
	}
...
func runDiscover(ctx context.Context, paths []string, recursive bool, maxDepth int, verbose bool) error {
	...
	if len(repos) > 0 {
		...
		fmt.Println()
		output.PrintInfo("Use 'wsm list repos' to see all repositories, or 'wsm create <name> --interactive' to create workspaces")
	}
