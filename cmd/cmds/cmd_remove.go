...
// NewRemoveCommand creates the service-based remove command
func NewRemoveCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Remove repositories from a workspace",
		Long: `Remove repositories from an existing workspace.
...
Examples:
  ...
  wsm remove my-feature old-repo

  # Remove multiple repositories from a workspace
  wsm remove my-feature repo1 repo2 repo3

  # Force remove repositories (removes worktrees even with uncommitted changes)
  wsm remove my-feature old-repo --force

  # Remove repositories and their directories from workspace
  wsm remove my-feature old-repo --remove-files

  # Dry run to see what would be removed
  wsm remove my-feature old-repo --dry-run`,
...
func runRemoveV2(ctx context.Context, workspaceName string, repoNames []string, force bool, removeFiles bool, outputFormat string, dryRun bool) error {
	...
	if len(updatedWorkspace.Repositories) == 0 {
		fmt.Printf("  💡 Workspace is now empty. Consider deleting it with: wsm delete %s\n", workspaceName)
	}
