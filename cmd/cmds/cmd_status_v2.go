...
func NewStatusCommandV2() *cobra.Command {
	var (
		short     bool
		untracked bool
		workspace string
	...
	cmd := &cobra.Command{
		Use:   "status-v2 [workspace-path]",
		Short: "Show workspace status using the new service architecture",
		Long: `Show the git status of all repositories in a workspace using the new service architecture.

The new architecture provides:
- Faster status checking with parallel repository scanning
- Better error handling and recovery
- Structured output with JSON support
- Detailed branch tracking and conflict detection
...
Examples:
  ...
  wsm status-v2 /path/to/workspace

  # Show compact status format
  wsm status-v2 --short

  # Include untracked files in output
  wsm status-v2 --untracked

  # JSON output for scripting
  wsm status-v2 --json

  # Verbose output with detailed information
  wsm status-v2 --verbose`,
